...
describe('middleware.generate.ts file', () => {
  ...
  describe('generateMiddleWare function', () => {
    ...
    it('should stop the process when not confirming', async () => {
      ...
      chai
        ...
        .to.deep.eq(0, 'console.log should not be called');
      chai
        .expect(consoleErrorStub.callCount)
        .to.deep.eq(0, 'console.error should not be called');
    });
    ...
    it('should log an error when the middleware shortcut is in use', async () => {
      ...
      chai
        .expect(consoleLogStub.callCount)
        .to.deep.eq(1, 'console.log should not be called');
      chai
        .expect(consoleErrorStub.callCount)
        .to.deep.eq(0, 'console.error should be called once');
      chai
      ...
      chai
        .expect(writeFileSyncStub.lastCall.args[1])
        .to.deep.eq(
  ...
  describe('generateMiddleWareHandler function', () => {
    ...
    it('should not prompt any questions and start generateMiddleWare function', async () => {
      ...
      chai
        .expect(consoleErrorStub.callCount)
        .to.deep.eq(0, 'should not log an error');
      chai
        .expect(generateMiddleWareStub.lastCall.args[0])
        .to.deep.eq('Test', 'should be Test');
      chai
        .expect(generateMiddleWareStub.lastCall.args[1])
        .to.deep.eq('T', 'should be T');
      chai
        .expect(generateMiddleWareStub.lastCall.args[2])
        .to.deep.eq('Empty', 'should be Empty');
    });
    ...
    it('should log an error and abort if not in garfe project', async () => {
      ...
      chai
        .expect(consoleErrorStub.callCount)
        .to.deep.eq(1, 'should not log an error');
      chai.expect(promptStub.callCount).to.deep.eq(0, 'should not prompt once');
      chai
        .expect(consoleErrorStub.calledOnceWith(messages.not_grafe))
        .to.be.eq(
