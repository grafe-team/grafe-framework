...
describe('route.generate.ts file', () => {
  ...
  describe('generateRoute function', () => {
    ...
    it('should log an error when not in grafe project', async () => {
      ...
      chai
        .expect(consoleErrorStub.calledWith(messages.not_grafe))
        .to.deep.eq(true, 'console.error should log not_grafe message');
    });
    ...
    it('should abort not using a given rest-method', async () => {
      ...
      chai
        .expect(
          consoleErrorStub.calledWith(messages.generateRoute.invalid_method)
        )
        .to.deep.eq(true, 'console.error should log invalid_method messgae');
    });
    ...
    it('should abort when route already exists', async () => {
      ...
      chai
        ...
        .to.deep.eq(1, 'prompt should be called once');
      chai
        .expect(consoleErrorStub.calledWith(messages.generateRoute.exists))
        .to.deep.eq(true, 'console.error should log exists messgae');
    });

    it('should abort when testfile exitsts', async () => {
      const newGrageConfig = JSON.parse(JSON.stringify(grafeConfig));
      newGrageConfig.middlewares.push({
        name: 'admin',
        description: 'just for tests',
        value: 'adm',
      });
      ...
      pkgDirStub.resolves('C:\\grafe\\project_1');
      readFileSyncStub.onFirstCall().returns(JSON.stringify(newGrageConfig));
      ...
      chai
        .expect(consoleErrorStub.calledWith(messages.generateRoute.exists))
        .to.deep.eq(true, 'should log the exists messgae');
    });
    ...
    it('should create route when everything is correct', async () => {
      ...
      chai
        ...
        .to.deep.eq(1, 'prompt should be called once');
      chai
        .expect(copyFileSyncStub.lastCall.args[1])
        .to.deep.eq('C:\\grafe\\project_1\\src\\routes\\_mw.pt\\test.get.ts');
    });
    ...
    it('should create route and test-file when everything is correct', async () => {
      ...
      chai
        .expect(consoleLogStub.callCount)
        .to.deep.eq(2, 'console.log should be called twice');
      chai
        .expect(promptStub.callCount)
        .to.deep.eq(1, 'prompt should be called once');
      chai
        .expect(writeFileSyncStub.lastCall.args[0])
        .to.deep.eq(
  ...
  describe('generateRouteHandler function', () => {
    ...
    let generateRouteHandler: (argv: Record<string, unknown>) => Promise<void> =
    ...
    it('should log an error when not in a grafe project', async () => {
      ...
      chai
        .expect(promptStub.callCount)
        .to.deep.eq(0, 'should not prompt the user');
      chai
        .expect(consoleErrorStub.calledWith(messages.not_grafe))
        .to.deep.eq(true, 'console.error should log not_grafe message');
    });
    ...
    it('should start generateRoute with given parameters', async () => {
      ...
      chai
        ...
        .to.deep.eq(0, 'should not prompt the user');
      chai
        .expect(generateRouteStub.lastCall.args[0])
        .to.deep.eq('/test', 'should be the given routePath');
      chai
        .expect(generateRouteStub.lastCall.args[1])
        .to.deep.eq('get', 'should be the given method');
      chai
        .expect(generateRouteStub.lastCall.args[2])
        .to.deep.eq([], 'should be the given middlewares');
    });
    ...
    it('should remove the middleware question', async () => {
      ...
      chai
        .expect(promptStub.callCount)
        .to.deep.eq(1, 'should not prompt the user');
      chai
        .expect(generateRouteStub.lastCall.args[0])
        .to.deep.eq('/test', 'should be the given routePath');
      chai
      ...
      chai
        .expect(generateRouteStub.lastCall.args[2])
        .to.deep.eq(undefined, 'should be the given middlewares');
    });
