...
describe('static.generate.ts file', () => {
    ...
    describe('generateStaticHandler function', () => { 
        ...
        let generateStaticHandler: (argv: any) => Promise<void>
                = staticGenerate.__get__('generateStaticHandler');
        ...
        it('should prompt the user when name not given', async () => {
            ...
            chai.expect(consoleErrorStub.callCount).to.deep.eq(0, 'console.error should not be called once');
            chai.expect(promptStub.callCount).to.deep.eq(1, 'user should be prompted once');
            chai.expect(generateStaticStub.lastCall.args[0]).to.deep.eq('Test', 'should be the string of the prompt');
        });
        ...
        it('should not prompt the user when name given', async () => {
            ...
            chai.expect(consoleErrorStub.callCount).to.deep.eq(0, 'console.error should not be called once');
            chai.expect(promptStub.callCount).to.deep.eq(0, 'user should not be prompted');
            chai.expect(generateStaticStub.lastCall.args[0]).to.deep.eq('Test', 'should be the string of the prompt');
        });
    });
});