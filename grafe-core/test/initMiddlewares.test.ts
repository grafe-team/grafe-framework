...
describe('initMiddlewares.ts file', () => {
    ...
    describe('removeNonExistantMiddleware function', () => {
        let removeNonExistantMiddleware: (
            config: Config,
            mw: Middleware
        ) => boolean;

        beforeEach(() => {
            removeNonExistantMiddleware = initMiddlewares.__get__(
                'removeNonExistantMiddleware'
            );
        });
        ...
        it('should throw an error because mw.link is undefined', () => {
            ...
            chai.expect(() => {
                removeNonExistantMiddleware(null, mw);
            }).to.throw(/name/);
        });
    ...
    describe('createMiddlewareLinks function', () => {
        ...
        beforeEach(() => {
            createMiddlewareLinks = initMiddlewares.__get__(
                'createMiddlewareLinks'
            );
        });
    ...
    describe('initMiddlewares function', () => {
        ...
        it('should load the function', () => {
            ...
            chai.expect(removeNonExistantMiddlewareStub.callCount).to.deep.eq(
                1
            );
            ...
            chai.expect(createMiddlewareLinksStub.lastCall.args[0]).to.deep.eq(
                config
            );
            chai.expect(createMiddlewareLinksStub.lastCall.args[1]).to.deep.eq(
                config.middlewares[0]
            );

            chai.expect(
                removeNonExistantMiddlewareStub.lastCall.args[0]
            ).to.deep.eq(config);
            chai.expect(
                removeNonExistantMiddlewareStub.lastCall.args[1]
            ).to.deep.eq(config.middlewares[0]);
        ...
        it('should not skip a middleware if one got removed', () => {
            ...
            removeNonExistantMiddlewareStub
                .onFirstCall()
                .callsFake(function fake() {
