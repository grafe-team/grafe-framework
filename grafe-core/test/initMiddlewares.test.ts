...
describe('initMiddlewares.ts file', () => {
  ...
  describe('initMiddlewares function', () => {
    ...
    it('should load the function', () => {
      ...
      chai
        .expect(createMiddlewareLinksStub.lastCall.args[0])
        .to.deep.eq(config);
      chai
        .expect(createMiddlewareLinksStub.lastCall.args[1])
        .to.deep.eq(config.middlewares[0]);

      chai
        .expect(removeNonExistantMiddlewareStub.lastCall.args[0])
        .to.deep.eq(config);
      chai
        .expect(removeNonExistantMiddlewareStub.lastCall.args[1])
        .to.deep.eq(config.middlewares[0]);
