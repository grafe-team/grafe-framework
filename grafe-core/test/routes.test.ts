...
describe('routes.ts file', () => {
    ...
    describe('populateMiddlewares function', () => {
        ...
        let populateMiddlewares: (mws: string[], allMiddlewares: Middleware[]) => Middleware[]
                 = routes.__get__('populateMiddlewares'); 
        ...
        it ('should populate the middlewares correctly without logging an error', () => {
            const middlewares = populateMiddlewares(['mw1', 'mw2'], allMiddlewares);

            chai.expect(consoleWarnStub.callCount).to.deep.eq(0, 'console.warn should not be called');
            chai.expect(middlewares).to.contain.members(allMiddlewares, 'middlwares should have the same members as allMiddlewares');

        });

        it ('should populate the middlewars corretly but log a warning because one mw does not exist', () => {
            const middlewares = populateMiddlewares(['mw1', 'mw3'], allMiddlewares);

            chai.expect(consoleWarnStub.callCount).to.deep.eq(1, 'console.warn should be called once');
            chai.expect(middlewares).to.contain(allMiddlewares[0], 'middlwares should only contain the first mw');
        });
        ...
        it ('should return an empty array if no mws are specified', () => {
            ...
            chai.expect(consoleWarnStub.callCount).to.deep.eq(0, 'console.warn should not be called');
            chai.expect(middlewares.length).to.deep.eq(0, 'middlewares should be empty');
        });
    ...
    describe('parseFileName function', () => {
        ...
        let parseFileName: (fileName: string, inheritedMws: Middleware[], allMiddlewares: Middleware[]) => { route: Route, ignored: boolean };

        let populateMiddlewaresStub = Sinon.stub();
        let parseRestMethodFromStringStub = Sinon.stub();
        let combineMiddlewareArraysStub = Sinon.stub();
        ...
        it ('should idicate that it ignores files that start with an "_"', () => {
            ...
            chai.expect(response.ignored).to.deep.eq(true, 'File should be marked as ignored');
            chai.expect(populateMiddlewaresStub.callCount).to.deep.eq(0, 'populateMiddlewares should not be called');
            chai.expect(parseRestMethodFromStringStub.callCount).to.deep.eq(0, 'parseResMethodFromString should not be called');
        });
        ...
        it ('should not ignore the file and create the endpoint for the function and call all neccessary functions', () => {
            ...
            chai.expect(response.ignored).to.deep.eq(false, 'The file should not be ignored');
            chai.expect(response.route.endpoint).to.deep.eq('test', 'The endpoint for the route should eq "test"');
            chai.expect(populateMiddlewaresStub.callCount).to.deep.eq(1, 'populateMiddlewares should be called once');
            chai.expect(parseRestMethodFromStringStub.callCount).to.deep.eq(1, 'parseRestMethodFromString should be called once');
        });
        ...
        it ('should properly combine the inherited middlewares with the parsed ones', () => {
            ...
            combineMiddlewareArraysStub.withArgs(allMiddlewares, allMiddlewares).returns(allMiddlewares);

            const result = parseFileName('test.post.js', allMiddlewares, allMiddlewares);

            chai.expect(result.route.middlewares).to.deep.eq(allMiddlewares, 'After the array was combined the array should not be edited anymore');
            chai.expect(combineMiddlewareArraysStub.callCount).to.deep.eq(1, 'combineMiddlewareArrays should be used to combine the arrays');
        });
        ...
        it ('should return the right rest method', () => {
            ...
            chai.expect(result.route.method).to.deep.eq('post', 'The returned rest method should be the same as returned from parseRestMethodFromString');
            chai.expect(parseRestMethodFromStringStub.callCount).to.deep.eq(1, 'parseRestMethodFromString should be called once');
        });
        ...
        it ('should add middlewares added in the filename', () => {
            ...
            chai.expect(populateMiddlewaresStub.calledWith(Sinon.match(["mw1"]))).to.deep.eq(true, 'Expect populate Middlewars to have been called with "mw1"');
        });
        ...
        it ('should add middlewares added in the filename with multible middlewares', () => {
            ...
            chai.expect(populateMiddlewaresStub.calledWith(Sinon.match(["mw1", "mw2", "mw3"]))).to.deep.eq(true, 'Expect populate Middlewars to have been called with ["mw1", "mw2", "mw3"]');
        });
    ...
    describe('combineMiddlewareArrays function', () => {
        ...
        let combineMiddlewareArrays: (array1: Middleware[], array2: Middleware[]) => Middleware[];
        ...
        it ('should combine both arrays without duplicating elements', () => {
            ...
            const combinedArray = combineMiddlewareArrays(allMiddlewares, allMiddlewares);

            chai.expect(combinedArray.length).to.deep.eq(2, 'The combined array should only have 2 elements');
            chai.expect(combinedArray).to.have.members(allMiddlewares, 'The combined array should have the same elements as allMiddlewares');
        });

        it ('should combine both arrays', () => {
            const combinedArray = combineMiddlewareArrays([allMiddlewares[0]], [allMiddlewares[1]]);

            chai.expect(combinedArray.length).to.deep.eq(2, 'The combined array should have a length of 2');
            chai.expect(combinedArray).to.have.members(allMiddlewares, 'The combined array should have the same elements as allMiddlewares');
        });
        ...
        it ('should not edit the original arrays', () => {
            ...
            chai.expect(array1).to.deep.eq(array1Cache, 'The first array should not be edited');
            chai.expect(array2).to.deep.eq(array2Cache, 'The second array should not be edited');
        });
    ...
    describe('parseRestMethodFromString function', () => {
        ...
        let parseRestMethodFromString: (stringMethod: string) => "post" | "get" | "put" | "delete" | "none";

        beforeEach(() => {
            parseRestMethodFromString = routes.__get__('parseRestMethodFromString');
        });
        ...
        it ('should not edit the original string', () => {
            ...
            chai.expect(string).to.deep.eq(stringCache, 'The orignial string should not be edited');
        });
        ...
        it ('should return the right rest method', () => {
            ...
            chai.expect(parseRestMethodFromString('delete')).to.deep.eq('delete');
        });
    ...
    describe('parseDirectoryName function', () => {
        ...
        let parseDirectoryName: (directoryName: string, inheritedMiddlewares: Middleware[], allMiddlewares: Middleware[]) => 
        ...
        it ('should detect the middlewares and return them', () => {
            ...
            chai.expect(combineMiddlewareArraysStub.calledAfter(populateMiddlewaresStub)).to.be.true;
        });
    ...
    describe ('_createRouteTree function', () => {
        ...
        let _createRouteTree: (parseDir: string, routePart: RoutePart, inheritedMiddlewares: Middleware[], allMiddlewares: Middleware[]) => void;
        ...
        it ('should try and parse the file name and add the route to the routeTree', () => {
            ...
            chai.expect(parseFileNameStub.lastCall.args[0]).to.deep.eq(fileInfo[0].name);
            chai.expect(parseFileNameStub.lastCall.args[1]).to.deep.eq([]);
            chai.expect(parseFileNameStub.lastCall.args[2]).to.deep.eq([]);
        
            chai.expect(readAllFilesStatsStub.lastCall.args[0]).to.deep.eq('test');
        });
        ...
        it ('should ignore the directory and not add it to the route tree', () => {
            ...
            chai.expect(parseDirectoryNameStub.lastCall.args[0]).to.deep.eq(fileInfo[0].name);
        ...
        it ('should parse the directory and insert it into the route tree', () => {
            ...
            chai.expect(readAllFilesStatsStub.firstCall.args[0]).to.deep.eq('test');
            chai.expect(readAllFilesStatsStub.lastCall.args[0]).to.deep.eq('/var/dirName');
        });
    ...
    describe('createRouteTree function', () => {
        ...
        it ('should try and create the route Tree and append it to the config', () => {
            ...
            chai.expect(_createRouteTreeStub.firstCall.args[0]).to.deep.eq(config.routePath);
    ...
    describe('createRouteTree integration test', () => {
        ...
        it ('should create a route tree', () => {
            ...
            chai.expect(config.routeTree.test).to.deep.eq({endpoint: 'test', method: 'post', middlewares: []});
            chai.expect(config.routeTree.dir1).to.have.all.keys('test2');
            
            chai.expect(config.routeTree.dir1.test2).to.deep.eq({endpoint: 'test2', method: 'get', middlewares: [config.middlewares[1]]})
        });
    });
