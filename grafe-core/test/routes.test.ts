...
describe('routes.ts file', () => {
  ...
  describe('populateMiddlewares function', () => {
    ...
    it('should populate the middlewares correctly without logging an error', () => {
      ...
      chai
        .expect(consoleWarnStub.callCount)
        .to.deep.eq(0, 'console.warn should not be called');
      chai
        .expect(middlewares)
        .to.contain.members(
          allMiddlewares,
    ...
    it('should populate the middlewars corretly but log a warning because one mw does not exist', () => {
      ...
      chai
        .expect(consoleWarnStub.callCount)
        .to.deep.eq(1, 'console.warn should be called once');
      chai
        .expect(middlewares)
        .to.contain(
          allMiddlewares[0],
    ...
    it('should return an empty array if no mws are specified', () => {
      ...
      chai
        ...
        .to.deep.eq(0, 'console.warn should not be called');
      chai
        .expect(middlewares.length)
        .to.deep.eq(0, 'middlewares should be empty');
    });
  });
  ...
  describe('parseFileName function', () => {
    ...
    it('should idicate that it ignores files that start with an "_"', () => {
      ...
      chai
        .expect(response.ignored)
        .to.deep.eq(true, 'File should be marked as ignored');
      chai
        .expect(populateMiddlewaresStub.callCount)
        .to.deep.eq(0, 'populateMiddlewares should not be called');
      chai
        .expect(parseRestMethodFromStringStub.callCount)
        .to.deep.eq(0, 'parseResMethodFromString should not be called');
    });
    ...
    it('should not ignore the file and create the endpoint for the function and call all neccessary functions', () => {
      ...
      chai
        .expect(response.ignored)
        .to.deep.eq(false, 'The file should not be ignored');
      chai
        .expect(response.route.endpoint)
        .to.deep.eq('test', 'The endpoint for the route should eq "test"');
      chai
        .expect(populateMiddlewaresStub.callCount)
        .to.deep.eq(1, 'populateMiddlewares should be called once');
      chai
        .expect(parseRestMethodFromStringStub.callCount)
        .to.deep.eq(1, 'parseRestMethodFromString should be called once');
    });
    ...
    it('should properly combine the inherited middlewares with the parsed ones', () => {
      ...
      chai
        .expect(result.route.middlewares)
        .to.deep.eq(
          allMiddlewares,
          'After the array was combined the array should not be edited anymore'
        );
      chai
        .expect(combineMiddlewareArraysStub.callCount)
        .to.deep.eq(
          1,
    ...
    it('should add middlewares added in the filename', () => {
      ...
      chai
        .expect(populateMiddlewaresStub.calledWith(Sinon.match(['mw1'])))
        .to.deep.eq(
          true,
    ...
    it('should add middlewares added in the filename with multible middlewares', () => {
      ...
      chai
        .expect(
          populateMiddlewaresStub.calledWith(Sinon.match(['mw1', 'mw2', 'mw3']))
        )
        .to.deep.eq(
          true,
  ...
  describe('combineMiddlewareArrays function', () => {
    ...
    it('should combine both arrays without duplicating elements', () => {
      ...
      chai
        .expect(combinedArray.length)
        .to.deep.eq(2, 'The combined array should only have 2 elements');
      chai
        .expect(combinedArray)
        .to.have.members(
          allMiddlewares,
    ...
    it('should combine both arrays', () => {
      ...
      chai
        .expect(combinedArray.length)
        .to.deep.eq(2, 'The combined array should have a length of 2');
      chai
        .expect(combinedArray)
        .to.have.members(
          allMiddlewares,
    ...
    it('should not edit the original arrays', () => {
      ...
      chai
        .expect(array1)
        .to.deep.eq(array1Cache, 'The first array should not be edited');
      chai
        .expect(array2)
        .to.deep.eq(array2Cache, 'The second array should not be edited');
    });
  });
  ...
  describe('parseRestMethodFromString function', () => {
    ...
    it('should not edit the original string', () => {
      ...
      chai
        .expect(string)
        .to.deep.eq(stringCache, 'The orignial string should not be edited');
    });
  ...
  describe('_createRouteTree function', () => {
    ...
    it('should try and parse the file name and add the route to the routeTree', () => {
      ...
      chai
        .expect(parseFileNameStub.lastCall.args[0])
        .to.deep.eq(fileInfo[0].name);
    ...
    it('should ignore the directory and not add it to the route tree', () => {
      ...
      chai
        .expect(parseDirectoryNameStub.lastCall.args[0])
        .to.deep.eq(fileInfo[0].name);
    ...
    it('should parse the directory and insert it into the route tree', () => {
      ...
      chai
        .expect(readAllFilesStatsStub.lastCall.args[0])
        .to.deep.eq('/var/dirName');
    });
